...
class MovableObject(ABC):
    ...
    @abstractmethod
    def _get_acceleration(self, **kwargs):
        pass

    def _move_euler(self, dt, **kwargs):
        self.velocity += self._get_acceleration(**kwargs) * dt
        self.position += self.velocity * dt
        self.time += dt

    def _move_runge_kutta(self, dt, **kwargs):
        order_weight = [1/6, 2/6, 2/6, 1/6]
        k_v, k_p = self._get_acceleration(**kwargs) * dt, self.velocity * dt
        diff_v, diff_p = k_v * order_weight[0], k_p * order_weight[0]

        for i in range(1, 4):
            k_v, k_p = self._get_acceleration(
